...
export default function ScheduledCapsulesPage() {
  ...
  const fetchCapsules = async () => {
    try {
      ...
      const scheduled = allCapsules.filter(c => !c.is_sent);
      setCapsules(scheduled);
    } catch (err: any) {
  ...
  const handleDelete = async (capsuleId: number) => {
    try {
      ...
      // Обновляем список капсул после удаления
      fetchCapsules();
    } catch (err: any) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Запланированные капсулы</CardTitle>
        <CardDescription>
          Здесь находятся все ваши капсулы, которые ожидают отправки.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
