...
# Определяем Generic типы для моделей и схем Pydantic
...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    :param model: Модель SQLAlchemy, с которой будет работать этот CRUD-класс.
    """
    ...
    def create(self, db: Session, *, obj_in: CreateSchemaType) -> ModelType:
        ...
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)  # Обновляем объект, чтобы получить данные из БД (например, ID)
        return db_obj
